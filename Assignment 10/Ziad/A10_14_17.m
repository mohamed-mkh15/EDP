...
for M=1:500
    ...
    X(1)=3500/sqrt(2);     %X initial value
    Y(1)=3500/sqrt(2);     %Y initial value
    Vx=-50;     %X velocity
    Vy=-45;     %Y velocity
    T=2;        %Time step
    ...
    beta(1)=atan2(Y(1),X(1));  %Azimuth initial value
    ...
    for i=2:N
        ...
        beta(i)=atan2(Y(i),X(i));      %Azimuth vector generation
    ...
    for i=1:N
        ...
        betai(i)=atan2(Xi(3),Xi(1));
        DiE(i)=sqrt(XiE(1)^2+XiE(3)^2);
        betaiE(i)=atan2(XiE(3),XiE(1));
...
plot(betai,xi)
%{
the graph shows clearly that cordinate position X dependance on the aximuth
angle beta can be assumed linear for small angles(beta less than 15
degrees). As beta increases the relation slightly turns into nonlinear but
still acceptable and the linearization errors could be neglected until beta
reaches 30 degreees. After that it loses its nonlinearity and it is very
importatnt in this case to take the liearity errors into consideration. 
%}