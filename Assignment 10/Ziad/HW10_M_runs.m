function [] = HW10_M_runs(no_of_runs,X1,Y1,sD,sB)
...
for M = 1:500
    ...
    sigma_d = sD;               % Range measurements noise standard deviation
    sigma_beta = sB;          % Azimuth measurements noise standard deviation
...
%Plot dependence of coordinate x on azimuth ?.
...
title('Dependence of coordinate x on azimuth Beta')
xlabel('Azimuth angle Beta (Degrees)')
ylabel('Distance x')

subplot(2,1,2)
plot(betai,xi)
title('Dependence of coordinate x on azimuth Beta')
xlabel('Azimuth angle Beta (Radians)')
ylabel('Distance x')
end