function [] = M_runs_smoothing(no_of_runs)
    N=200;              % size of trajectory
    err_run_X = zeros(N-2,no_of_runs);
    err_run_V = zeros(N-2,no_of_runs);
    err_run_Xs = zeros(N-3,no_of_runs);
    err_run_Vs = zeros(N-3,no_of_runs);
    ...
    for i = 1:N-3
        ...
        error_true_Xs(i) = sqrt((1/(no_of_runs-1)*sum(err_run_Xs(i,:))));
        error_true_Vs(i) = sqrt((1/(no_of_runs-1)*sum(err_run_Vs(i,:))));
    end
